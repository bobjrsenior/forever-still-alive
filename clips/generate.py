...
def nextClip(groupId, clipId, randomize):
    ...
    if randomize:
        ...
        # If we just played the last clip in a group, transition to another group
...
def generate_file(iterations, randomize, ender):
    ...
    # randomize: determines if we try to group clips with a similar beat together
    # ender: determines if we end the generated list with the normal ending clip of the song
...
def call_ffmpeg():
    ...
    subprocess.call(['ffmpeg', '-f', 'concat', '-safe', '0', '-i', 'filelist-generated', '-c', 'copy', '-y', 'Still-Alive-Generated' + extension])

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generates a random version of Still Alive.")
    parser.add_argument('iterations', metavar='ITERATIONS', type=int, help='minimum number of clips to add')
    parser.add_argument('-r', '--random', help='Makes the clip selection completely random instead of organized by tempo', action='store_true')
    parser.add_argument('-n', '--normalEnd', help='Makes the last clip the normal song end clip instead of a random one', action='store_true')
